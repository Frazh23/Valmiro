...
export default function StatisticheTraffico({utente,abilitato}:{utente:string;abilitato:boolean}){
 ...
 useEffect(()=>{let vivo=true;setDati(null);setErrore('');const sb=supabase();
 ...
 useEffect(()=>{const aggiorna=()=>{setDati(null);setRefresh(x=>x+1);};window.addEventListener('focus',aggiorna);const timer=setInterval(aggiorna,60000);return()=>{clearInterval(timer);window.removeEventListener('focus',aggiorna);};},[]);
 ...
 {dati.pagineViste===0?<p>Nessuna visita rilevata nel periodo selezionato.</p>:<><h3 className="v-h3">Visite giorno per giorno</h3><div className="v-traffico-grafico" role="img" aria-label={dati.giorni.map(g=>`${g.giorno}: ${g.visite} visite`).join('; ')}>{dati.giorni.map(g=><div key={g.giorno} title={`${g.giorno}: ${g.visite} visite, ${g.pagine} pagine`}><span style={{height:`${g.visite/picco*100}%`}}/><small>{g.giorno.slice(8)}</small></div>)}</div>
