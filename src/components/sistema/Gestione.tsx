...
import StatisticheTraffico from "@/components/sistema/StatisticheTraffico";
 ...
 *
 * Qui c'e' solo cio' che il database sa davvero. Le visite richiedono consenso; le
...
export default function Gestione({raccoltaAttiva=false}:{raccoltaAttiva?:boolean}) {
  ...
  useEffect(() => {
    ...
    sb.rpc("metriche_gestione").then(({ data, error }) => {
      ...
      setM(data as Metriche); setProprietario(utente.id); setStato("ok");
    });
    return () => { vivo=false; };
  ...
  if (!m || proprietario!==utente?.id) return <Pagina>{null}</Pagina>;
  ...
  return (
    <Pagina>
      ...
      <StatisticheTraffico key={utente!.id} utente={utente!.id} abilitato={raccoltaAttiva} />
      ...
      <ul className="v-ipotesi v-measure" style={{ marginTop: "var(--s-3)" }}>
        <li><b>Le visite senza consenso non compaiono.</b> Le statistiche descrivono solo il traffico misurato, non tutte le persone che arrivano sul sito.</li>
