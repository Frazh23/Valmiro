...
export function leggiAnnuncio(testo: string): Letto {
  ...
  // ---- balconi e terrazzi
  const b = l.match(/(\d)\s*balcon/);
  if (b) out.balconi = Number(b[1]);
  else if (/balcon|terrazz/.test(l)) out.balconi = 1;
  if (out.balconi) out.trovati.push(out.balconi === 1 ? "un balcone o terrazzo" : `${out.balconi} balconi`);
