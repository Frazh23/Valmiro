...
exports.PIANO_NON_VALUTABILE = exports.PARAMETRI = exports.COMPRESSIONE_STATO = exports.ONERI = exports.DETRAZIONE = exports.MOLTIPLICATORE_RISTRUTTURAZIONE = exports.RISTRUTTURAZIONE = exports.COEFF = void 0;
exports.scala = scala;
exports.baseOmi = baseOmi;
exports.pertinenzePonderate = pertinenzePonderate;
exports.superficie = superficie;
exports.superficieCommerciale = superficieCommerciale;
exports.stima = stima;
const data_1 = require("./data");
const provenienza_1 = require("./provenienza");
/* --------------------------------------------------------------------------
   Motore di stima.
...
exports.RISTRUTTURAZIONE = [
    { id: "base", nome: "Essenziale", statoDopo: "otti" },
    { id: "completa", nome: "Completa", statoDopo: "nuov" },
    { id: "design", nome: "Design", statoDopo: "nuov" },
];
/** Il costo dei lavori dipende dalla fascia: accessi, ponteggi, livello di finitura atteso. */
exports.MOLTIPLICATORE_RISTRUTTURAZIONE = { B: 1.2, C: 1.08, D: 0.98, E: 0.92, R: 0.98 };
/**
 ...
 */
exports.DETRAZIONE = { primaCasa: 0.5, altri: 0.36, tetto: 96000, rate: 10 };
/**
 ...
 */
exports.COMPRESSIONE_STATO = 0.7;
/**
 ...
 */
exports.PARAMETRI = {
    compressioneStato: { B: 0.45, C: 0.45, D: 0.7, E: 0.7, R: 0.7 },
    ...
    livello: { B: 1.02, C: 1.05, D: 1.0, E: 1.0, R: 1.0 },
...
const fasciaDi = (zona) => data_1.ZONE[zona]?.f || "D";
/** Le due fasce OMI ridotte alle loro mediane, con i ripieghi per le zone incomplete. */
function scala(zona, tipo) {
    const z = data_1.ZONE[zona];
    if (!z)
        throw new Error(`Zona OMI sconosciuta: ${zona}`);
    let t = z[tipo];
...
function baseOmi(zona, tipo, stato) {
    const s = scala(zona, tipo);
    const f = fasciaDi(zona);
    const premio = Math.pow(s.mediaO / s.mediaN, exports.PARAMETRI.compressioneStato[f] ?? exports.COMPRESSIONE_STATO);
    if (stato === "rist")
        return s.mediaN * exports.PARAMETRI.scontoRist;
    if (stato === "abit")
        return s.mediaN;
    if (stato === "otti")
        return s.mediaN * premio;
    return s.mediaN * premio * 1.06;
}
/** Metri commerciali che balconi e terrazzi aggiungono, con la regola del DPR 138/98. */
function pertinenzePonderate(mqBalconi = 0, mqTerrazzi = 0) {
    const tot = Math.max(0, mqBalconi || 0) + Math.max(0, mqTerrazzi || 0);
    return Math.min(tot, exports.COEFF.pertinenzeSoglia) * exports.COEFF.pertinenzeQuota1
        + Math.max(0, tot - exports.COEFF.pertinenzeSoglia) * exports.COEFF.pertinenzeQuota2;
}
/** La superficie commerciale scomposta: cosi' il dettaglio puo' dire da dove viene ogni metro. */
function superficie(i) {
    const calpestabile = i.superficie === "calpestabile";
    const principale = calpestabile ? i.mq * exports.COEFF.muri : i.mq;
...
const PIANO_NON_VALUTABILE = (p) => `Per il piano ${p} non è disponibile una valutazione attendibile: il modello attuale non dispone di un trattamento validato per questo piano. Si può chiedere una simulazione che ipotizza un piano terra, sapendo che non è una valutazione del ${p}.`;
exports.PIANO_NON_VALUTABILE = PIANO_NON_VALUTABILE;
function stima(input) {
    const z = data_1.ZONE[input.zona];
    if (!z)
        throw new Error(`Zona OMI sconosciuta: ${input.zona}`);
    if (!(input.mq > 0))
        throw new Error("Superficie mancante o non valida");
    ...
    if (input.pianoDichiarato) {
        if (!input.simulazionePiano)
            throw new Error((0, exports.PIANO_NON_VALUTABILE)(input.pianoDichiarato));
        simulazione = {
    ...
    const materiali = input.versioneProvenienza === 2 ? [] : (0, provenienza_1.ipotesiMateriali)(input);
    if (materiali.length && !input.simulazioneDati)
        throw new Error((0, provenienza_1.DATI_NON_CONFERMATI)(materiali));
    ...
    const ipotesi = materiali.length ? (0, provenienza_1.ipotesiDi)(input).map(provenienza_1.descriviIpotesi) : undefined;
    const noteDati = input.versioneProvenienza !== 2 && !materiali.length && (0, provenienza_1.ipotesiDi)(input).length
        ? (0, provenienza_1.ipotesiDi)(input).map((x) => `${(0, provenienza_1.descriviIpotesi)(x)}; al valore predefinito non sposta il valore`)
        : undefined;
    const sc = superficieCommerciale(i);
    const base = baseOmi(i.zona, i.tipo, i.stato) * data_1.INDICE_ISTAT * (exports.PARAMETRI.livello[fasciaDi(i.zona)] ?? 1);
    const grezzo = sc * base;
    const alto = ["3-5", "6+", "ultimo"].includes(i.piano);
    const k = {
        piano: exports.COEFF.piano[i.piano],
        ascensore: i.ascensore ? 1 : alto ? exports.COEFF.senzaAscensoreAlto : exports.COEFF.senzaAscensoreBasso,
        classe: i.classe === "nd" ? 1 : exports.COEFF.classe[i.classe],
        luce: exports.COEFF.luce[i.luce || "media"],
        epoca: i.epoca ? exports.COEFF.epoca[i.epoca] : 1,
        affaccio: i.affaccio ? exports.COEFF.affaccio[i.affaccio] : 1,
        metro: i.metro ? exports.COEFF.metro[i.metro] : 1,
    };
    const kTot = Object.values(k).reduce((a, b) => a * b, 1);
    const boxUnit = z.box ? (z.box[0] + z.box[1]) / 2 : 1200;
    const extra = i.box === "box" ? boxUnit * exports.COEFF.boxMq
        : i.box === "posto" ? boxUnit * exports.COEFF.boxMq * exports.COEFF.postoAutoSuBox
            : 0;
    ...
    const sigmaBase = Math.max(0.03, Math.min(0.13, 0.045 + dispersione * 0.35 + sigmaStato + (i.mq > 160 ? 0.015 : 0) - affinate * 0.008));
    const pregio = i.tipo === "sig";
    const sigma = pregio ? Math.min(0.2, sigmaBase + exports.PARAMETRI.incertezzaSig) : sigmaBase;
    ...
    const baseNormale = baseOmi(i.zona, i.tipo, "abit") * data_1.INDICE_ISTAT * (exports.PARAMETRI.livello[fasciaDi(i.zona)] ?? 1);
    ...
    if (sup.balconiTerrazzi) {
        ...
        dettaglio.push({ voce: `Balconi e terrazzi · ${Math.round(mq)} mq contati al ${mq > exports.COEFF.pertinenzeSoglia ? "30% fino a 25 e al 10% oltre" : "30%"} (DPR 138/98)`, effetto: 0, euro: sup.balconiTerrazzi * base });
    }
    if (sup.cantina)
        dettaglio.push({ voce: "Cantina o soffitta · 2,5 mq commerciali", effetto: 0, euro: sup.cantina * base });
    ...
    dettaglio.push(voce(simulazione ? `Piano terra · ipotesi della simulazione, il piano dichiarato è ${simulazione.pianoDichiarato}` : `Piano ${i.piano}`, k.piano), voce(i.ascensore ? "Ascensore presente" : "Senza ascensore", k.ascensore), i.classe === "nd"
        ? { voce: "Classe energetica non dichiarata · nessun aggiustamento", effetto: 0, euro: 0, nota: true }
        : voce(`Classe energetica ${i.classe}`, k.classe), voce(`Luminosità ${i.luce || "media"}`, k.luce));
    if (i.epoca)
        dettaglio.push(voce(`Epoca ${i.epoca}`, k.epoca));
    if (i.affaccio)
        dettaglio.push(voce(`Affaccio ${i.affaccio}`, k.affaccio));
    if (i.metro)
        dettaglio.push(voce(`Metropolitana ${i.metro}`, k.metro));
    if (extra)
        dettaglio.push({ voce: i.box === "box" ? "Box auto" : "Posto auto", effetto: 0, euro: extra });
    ...
    return {
        ...
        pubblica: arrotonda(totale * (1 + exports.COEFF.margineTrattativa)),
        offerta: arrotonda(totale * (1 - exports.COEFF.margineTrattativa)),
        euroMq: soloCasa / sc,
        ...
        abitazione: {
            ...
            pubblica: arrotonda(soloCasa * (1 + exports.COEFF.margineTrattativa)),
        },
        simulazione,
        ipotesi,
        noteDati,
        ipotesiUsate: input.versioneProvenienza === 2 ? (0, provenienza_1.noteIpotesi)(input) : undefined,
        origineDatiRegistrata: !!input.provenienza,
        dettaglio,
        semestre: data_1.SEMESTRE,
        fonte: data_1.FONTE,
    };
}
